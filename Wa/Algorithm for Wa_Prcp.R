...
Prcp.xts.ann <- apply.yearly(Prcp.xts, sum)
head(Prcp.xts.ann)
...
write.csv(Prcp.df.ann, 
          file="Annual Rainfall of Wa (1960-2016).csv")
...
ggplot(Prcp.df.ann, aes(x=date, y=Prcp)) + geom_line(col="darkblue", lwd=2) + 
  ...
  annotate("text", x= Prcp.df.ann[Prcp.df.ann[,1] == "1988-12-31", ]$date, 
            y = 1400, label = "P-value = 0.6153", cex = 5) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, 
         filename="Annual Rainfall of Wa (1960-2016).png", width=1000, height=750)
dev.off()

sum12 <- summary(Prcp.df.ann)
write.csv(sum12,
          file = "summary stats_11 for Wa Ann. Rainfall (1960-2016).csv")
#or
sum1 <- data.frame(
  Prcp.df.ann[
    Prcp.df.ann[,2]==max(Prcp.df.ann[,2]),],
  Prcp.df.ann[
    Prcp.df.ann[,2]==min(Prcp.df.ann[,2]),]
)

sum1[2,c(1,2)] <- sum1[1, c(3:4)] 
sum1[-c(3,4)] -> sum1
...
write.csv(sum1,
          file = "summary stats_1 for Wa Ann. Rainfall (1960-2016).csv")
...
write.csv(mean.mn.Prcp, 
          file="Mean Monthly Rainfall of Wa (1960-2016).csv")
...
sum2 <- data.frame(
  mean.mn.Prcp[
    mean.mn.Prcp[,2]==max(mean.mn.Prcp[,2]),],
  mean.mn.Prcp[
    mean.mn.Prcp[,2]==min(mean.mn.Prcp[,2]),]
)

sum2[2,c(1,2)] <- sum2[1, c(3:4)] 
sum2[-c(3,4)] -> sum2
sum2[,1] <- data.frame(month.name[c(08,01)])
dimnames(sum2) <- list(
  c("Max:","Min:"), 
  c("Month","Rainfall")
) 
write.csv(sum2,
          file = "summary stats_1 for Wa Mnth. Rainfall (1960-2016).csv")




#Annual Rainy Days of Wa
...
Ann.rndys.v <- c(b.60.1,b.61.1, b.62.1, b.63.1, b.64.1,b.65.1,b.66.1, b.67.1, b.68.1 ,b.69.1,
                 ...
                 b.80.1, b.81.1, b.82.1, b.83.1, b.84.1, b.85.1, b.86.1, b.87.1, b.88.1, b.89.1, 
                 b.90.1, b.91.1, b.92.1, b.93.1, b.94.1, b.95.1, b.96.1, b.97.1, b.98.1, b.99.1,
                 b.00.1, b.01.1, b.02.1, b.03.1, b.04.1, b.05.1, b.06.1, b.07.1, b.08.1, b.09.1, 
                 b.10.1, b.11.1, b.12.1, b.13.1, b.14.1, b.15.1, b.16.1)
...
write.csv(Ann.rndys, 
          file="Annual Rainy Days of Wa (1960-2016).csv")
...
ggplot(Ann.rndys, aes(x=date.ann, y=Ann.rndys.v)) + 
  ...
  labs(title="Number of Days when Rainfall >= 0.85mm (1960-2016)\n Wa", 
       x="Year",y="Day") + 
  geom_smooth(method = lm, col="red") + 
  annotate("text", x = Ann.rndys[Ann.rndys[,1] == "1980-01-01", ]$date.ann, 
             y = 90, label="P-value = 0.056235", cex = 5) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, 
         filename="Annual Rainy Days of Wa (1960-2016).png", 
         width = 1000, height = 750)
dev.off()

sum3 <- data.frame(
  Ann.rndys[
    Ann.rndys[,2]==max(Ann.rndys[,2]),],
  Ann.rndys[
    Ann.rndys[,2]==min(Ann.rndys[,2]),]
)

sum3[2,c(1,2)] <- sum3[1, c(3:4)] 
sum3[-c(3,4)] -> sum3
#sum3[,1] <- data.frame(month.name[c(08,01)])
dimnames(sum3) <- list(
  c("Max:","Min:"), 
  c("Year","Rainfall")
) 
write.csv(sum3,
          file = "summary stats_1 for Wa Ann. rainydays (1960-2016).csv")



# Annual Rainy Days of Wa >= 20mm
head(Prcp.xts.df)
...
Ann.rndys20.v <- c(b.60.2, b.61.2,b.62.2, b.63.2, b.64.2,b.65.2,b.66.2, b.67.2, b.68.2 ,
                   b.69.2, b.70.2, b.71.2, b.72.2, b.73.2, b.74.2, b.75.2, b.76.2, b.77.2, 
                   b.78.2, b.79.2, b.80.2, b.81.2, b.82.2, b.83.2, b.84.2, b.85.2, b.86.2, 
                   b.87.2, b.88.2, b.89.2, b.90.2, b.91.2, b.92.2, b.93.2, b.94.2, b.95.2, 
                   b.96.2, b.97.2, b.98.2, b.99.2, b.00.2, b.01.2, b.02.2, b.03.2, b.04.2, 
                   b.05.2, b.06.2, b.07.2, b.08.2, b.09.2, b.10.2, b.11.2, b.12.2, b.13.2, 
                   b.14.2, b.15.2, b.16.2)
...
Ann.rdys20.Wa <- data.frame(
  date.rdys20, 
  Ann.rndys20.v
  )

MK <- MannKendall(Ann.rdys20.Wa[,2])
summary(MK)

write.csv(Ann.rdys20.Wa, 
          file="Annual Rainy Days of Wa grt than 20mm (1960-2016).csv")

ggplot(Ann.rdys20.Wa, aes(x=date.rdys20, y=Ann.rndys20.v)) + 
  geom_line(col="darkblue", lwd=2) + 
  geom_smooth(method = lm, col="red") + 
  scale_x_date(date_breaks = "4 years", date_labels = "%Y") + 
  ggtitle("Number of Days when Rainfall >= 20mm (1960-2016)\n Wa") + 
  xlab("Year") +ylab("Rainfall(mm)") + 
  annotate("text", x = Ann.rdys20.Wa[Ann.rdys20.Wa$date.rdys20 == "1977-01-01", ]$date.rdys20, 
           y = 28, label = "P-value = 0.14858", cex = 5) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, 
         filename="Annual Rainy Days of Wa grt 20mm (1960-2016).png", 
         width = 1000, height = 750)
dev.off()

sum4 <- data.frame(
  Ann.rdys20.Wa[
    Ann.rdys20.Wa[,2]==max(Ann.rdys20.Wa[,2]),],
  Ann.rdys20.Wa[
    Ann.rdys20.Wa[,2]==min(Ann.rdys20.Wa[,2]),]
)

sum4[2,c(1,2)] <- sum4[1, c(3:4)] 
sum4[-c(3,4)] -> sum4
#sum4[,1] <- data.frame(month.name[c(08,01)])
dimnames(sum4) <- list(
  c("Max:","Min:"), 
  c("Year","Rainfall")
) 
write.csv(sum4,
          file = "summary stats_1 for Wa Ann.Ranfall grt than 20mm (1960-2016).csv")
...
Prcp.mn.rdys <- Prcp.1[Prcp.1$Prcp >= 0.85, ]
...
Wa.ann.rdys <- c(J,F,M,A,Ma,Ju,Jl,Au,S,O,N,D)
date.mn
...
write.csv(monthly.rdys, 
          file = "Monthly Rainy Days of Wa (1960-2016).csv")

ggplot(monthly.rdys, aes(x=date.mn, y=Wa.ann.rdys)) + 
  geom_line(col="darkblue", lwd=2) + 
  labs(title="Number of Days when Rainfall >= 0.85mm (1960-2016)\n Wa",
       x="Month", y="Rainfall(mm)") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
...
dev.copy(png, 
         filename="Monthly Rainy Days of Wa (1960-2016)", 
         height = 500, width = 1000)
dev.off()

sum5 <- data.frame(
  monthly.rdys[
    monthly.rdys[,2]==max(monthly.rdys[,2]),],
  monthly.rdys[
    monthly.rdys[,2]==min(monthly.rdys[,2]),]
)

sum5[2,c(1,2)] <- sum5[1, c(3:4)] 
sum5[-c(3,4)] -> sum5
sum5[,1] <- data.frame(month.name[c(09,01)])
dimnames(sum5) <- list(
  c("Max:","Min:"), 
  c("Month","Rainfall")
) 
write.csv(sum5,
          file = "summary stats_1 for Wa Mnth. Rain days grt 0.85mm (1960-2016).csv")
...
Prcp.mn.rdys20 <- Prcp.1[Prcp.1$Prcp >= 20, ]
...
#creating a vector with the aboves
Wa.ann.rdys20 <- c(J.1,F.1,M.1,A.1,Ma.1,Ju.1,
...
write.csv(monthly.rdys20, 
          file = "Monthly Rainy Days of Wa grt 20 (1960-2016).csv")

ggplot(monthly.rdys20, aes(x=date.mn, y=Wa.ann.rdys20)) + 
  geom_line(col="darkblue", lwd=2) + 
...
dev.copy(png, 
         filename="Monthly Rainy Days of Bole (1960-2017)", 
         height = 750, width = 1000)
dev.off()

sum6 <- data.frame(
  monthly.rdys20[
    monthly.rdys20[,2]==max(monthly.rdys20[,2]),],
  monthly.rdys20[
    monthly.rdys20[,2]==min(monthly.rdys20[,2]),]
)

sum6[2,c(1,2)] <- sum6[1, c(3:4)] 
sum6[-c(3,4)] -> sum6
sum6[,1] <- data.frame(month.name[c(08,11)])
dimnames(sum6) <- list(
  c("Max:","Min:"), 
  c("Month","Rainfall")
) 
write.csv(sum6,
          file = "summary stats_1 for Wa Mnth.raidays grt 20mm (1960-2016).csv")



#Indexing Annual Rainfall (Wa)
head(Prcp.df.ann)
...
write.csv(Prcp.df.ann.anom, 
          file="Index of Annual Rainfall of Wa (1960-2017).csv")
...
ggplot(Prcp.df.ann.anom, aes(date.ann.1, Prcp.df.ann.anom.v, ymin = 0,
  ...
  labs(title="Annual Rainfall Anomaly (1960-2016)\n Wa", x="Year", y="Anomaly") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, 
         filename="Index of Annual Rainfall of Wa (1960-2016)", 
         height = 750, width = 1000)
dev.off()

sum7 <- data.frame(
  Prcp.df.ann.anom[
    Prcp.df.ann.anom[,2]==max(Prcp.df.ann.anom[,2]),],
  Prcp.df.ann.anom[
    Prcp.df.ann.anom[,2]==min(Prcp.df.ann.anom[,2]),]
)

sum7[2,c(1,2)] <- sum7[1, c(3:4)] 
sum7[-c(3,4)] -> sum7
#sum7[,1] <- data.frame(month.name[c(08,01)])
dimnames(sum7) <- list(
  c("Max:","Min:"), 
  c("Year","Anomaly")
) 
write.csv(sum7,
          file = "summary stats_1 for Wa Ann. Rainfall Index (1960-2016).csv")



#Seasonal Cycle of Rainfall of Wa (1960-2016)
head(Prcp)
#let Prcp,
data <- Prcp
...
mean_daily_wa<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11, 
                  Jan_12,Jan_13,Jan_14,Jan_15,Jan_16,Jan_17,Jan_18,Jan_19,Jan_20,Jan_21,
                  Jan_22,Jan_23,Jan_24,Jan_25,Jan_26,Jan_27,Jan_28,Jan_29,Jan_30,Jan_31,Feb_32,Feb_33,
                  Feb_34,Feb_35,Feb_36,Feb_37,Feb_38,Feb_39,Feb_40,Feb_41,Feb_42,Feb_43, Feb_44,Feb_45,
                  Feb_46,Feb_47,Feb_48,Feb_49,Feb_50,Feb_51,Feb_52,Feb_53,Feb_54,Feb_55,Feb_56,Feb_57,
                  Feb_58,Feb_59,Feb_59_1,Mar_60, Mar_61,Mar_62,Mar_63,Mar_64,Mar_65,Mar_66,Mar_67,Mar_68,
                  Mar_69,Mar_70,Mar_71,Mar_72,Mar_73,Mar_74,Mar_75,Mar_76,Mar_77,Mar_78,Mar_79,Mar_80,Mar_81,
                  Mar_82,Mar_83,Mar_84,Mar_85,Mar_86,Mar_87,Mar_88,Mar_89,Mar_90,Apr_91,Apr_92,Apr_93,Apr_94,Apr_95,
...
Seasonal_cycle_wa <-data.frame(
  c(1:366),
  mean_daily_wa
  )

write.csv(Seasonal_cycle_wa, 
...
ggplot(Seasonal_cycle_wa, aes(x=c(1:366), y=mean_daily_wa)) + 
...
dev.copy(png, 
         filename="Seasonal Cycle of Rainfall of Wa (1960-2016)", 
         height = 500, width = 1000)
dev.off()

sum8 <- data.frame(
  Seasonal_cycle_wa[
    Seasonal_cycle_wa[,2]==max(Seasonal_cycle_wa[,2]),],
  Seasonal_cycle_wa[
    Seasonal_cycle_wa[,2]==min(Seasonal_cycle_wa[,2]),]
)

sum8[2,c(1,2)] <- sum8[1, c(3:4)] 
sum8[-c(3,4)] -> sum8
#sum8[,1] <- data.frame(month.name[c(08,01)])
dimnames(sum8) <- list(
  c("Max:","Min:"), 
  c("Day","Rainfall")
) 
...
write.csv(Prcp_fh,file="Annual Rainfall Total of Wa(1960-1988).csv")

Prcp_sh <- Prcp.df.ann[(Prcp.df.ann$date >= "1989-12-31" & 
                          Prcp.df.ann$date <= "2016-12-31"), ]
write.csv(Prcp_sh,file="Annual Rainfall TOtal of Wa (1989-2016).csv")
...
#geom_smooth(method = lm, col="red") + 
 # labs(title='ANNUAL Rainfall of Bole (1960-1988)', x="Years", y="Rainfall(mm)") 
...
write.csv(mean.mn.Prcpfh, 
          file="Mean Monthly Rainfall of Wa (1960-1987).csv")
...
sum9 <- data.frame(
  mean.mn.Prcpfh[
    mean.mn.Prcpfh[,2]==max(mean.mn.Prcpfh[,2]),],
  mean.mn.Prcpfh[
    mean.mn.Prcpfh[,2]==min(mean.mn.Prcpfh[,2]),]
)

sum9[2,c(1,2)] <- sum9[1, c(3:4)] 
sum9[-c(3,4)] -> sum9
sum9[,1] <- data.frame(month.name[c(09,01)])
dimnames(sum9) <- list(
  c("Max:","Min:"), 
  c("Month","Rainfall")
) 
write.csv(sum9,
          file = "summary stats_1 for Wa Mnth. Rainfall (1960-1988).csv")
...
write.csv(mean.mn.Prcpsh, 
          file="Mean Monthly Rainfall of Wa (1988-2016).csv")
...
dev.copy(png, filename="Mean Monthly Rainfall of Wa (1989-2016).png",
         width=1000, height=750)
dev.off()

sum10 <- data.frame(
  mean.mn.Prcpsh[
    mean.mn.Prcpsh[,2]==max(mean.mn.Prcpsh[,2]),],
  mean.mn.Prcpsh[
    mean.mn.Prcpsh[,2]==min(mean.mn.Prcpsh[,2]),]
)

sum10[2,c(1,2)] <- sum10[1, c(3:4)] 
sum10[-c(3,4)] -> sum10
sum10[,1] <- data.frame(month.name[c(08,12)])
dimnames(sum10) <- list(
  c("Max:","Min:"), 
  c("Month","Rainfall")
) 
write.csv(sum10,
          file = "summary stats_1 for Wa Mnth. Rainfall (1989-2016).csv")


#plot(mean.mn.Prcpfh,col="darkblue" ,lwd=3, type="l",ylim=c(0,250) ,
#     main="Mean Monthly Rainfall of Wa - 1960-2017", 
#     xlab="Months", ylab="Rainfall(mm)", lty=1)
...
dev.copy(png, filename="Mean Monthly Rainfall of Wa (1960-88-2017).png", 
         width = 1000, height = 750)
dev.off()
...
ggplot(data = data.frame(c(1:29), Ann.rndys_fh[,2]),aes(x=c(1:29))) +
  ...
  scale_color_manual("",values = c("1960-1988"="darkblue",
                                   "1989-2016"="red")) +
  labs(title = "Comparing Number of Days when Rainfall >= 0.87 (1960-2016)\n Wa",
       x="X", y="Day") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))
...
Ann.rndys20_fh <- Ann.rdys20.Wa[Ann.rdys20.Wa[,1] >= "1960-01-01" &
                                        Ann.rdys20.Wa[,1] <= "1988-01-01", ]

Ann.rndys20_sh <- Ann.rdys20.Wa[Ann.rdys20.Wa[,1] >= "1989-01-01" &
                                        Ann.rdys20.Wa[,1] <= "2016-01-01", ]
...
mean_daily_wafh<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11,
                    Jan_12,Jan_13,Jan_14,Jan_15,Jan_16,Jan_17,Jan_18,Jan_19,Jan_20,Jan_21,
                    ...
                    Mar_71,Mar_72,Mar_73,Mar_74,Mar_75,Mar_76,Mar_77,Mar_78,Mar_79,Mar_80,Mar_81,Mar_82,Mar_83,Mar_84,Mar_85,Mar_86,Mar_87,Mar_88,Mar_89,Mar_90,Apr_91,Apr_92,Apr_93,Apr_94,Apr_95,Apr_96,Apr_97,Apr_98,Apr_99,Apr_100,Apr_101,Apr_102,Apr_103,Apr_104,Apr_105,Apr_106,Apr_107,Apr_108,Apr_109,Apr_110,Apr_111,Apr_112,Apr_113,Apr_114,Apr_115,Apr_116,Apr_117,Apr_118,Apr_119,Apr_120,May_121,May_122,May_123,May_124,May_125,May_126,May_127,May_128,May_129,May_130,May_131,May_132,May_133,May_134,May_135,May_136,May_137,May_138,May_139,May_140,May_141,May_142,May_143,May_144,May_145,May_146,May_147,May_148,May_149,May_150,May_151,Jun_152,Jun_153,Jun_154,Jun_155,Jun_156,Jun_157,Jun_158,Jun_159,Jun_160,Jun_161,Jun_162,Jun_163,Jun_164,Jun_165,Jun_166,Jun_167,Jun_168,Jun_169,Jun_170,Jun_171,Jun_172,Jun_173,Jun_174,Jun_175,Jun_176,Jun_177,Jun_178,Jun_179,Jun_180,Jun_181,Jul_182,Jul_183,Jul_184,Jul_185,Jul_186,Jul_187,Jul_188,Jul_189,Jul_190,Jul_191,Jul_192,Jul_193,Jul_194,Jul_195,Jul_196,Jul_197,Jul_198,Jul_199,Jul_200,Jul_201,Jul_201,Jul_203,Jul_204,Jul_205,Jul_206,Jul_207,Jul_208,Jul_209,Jul_210,Jul_211,Jul_212,Aug_213,Aug_214,Aug_215,Aug_216,Aug_217,Aug_218,Aug_219,Aug_220,Aug_221,Aug_222,Aug_223,Aug_224,Aug_225,Aug_226,Aug_227,Aug_228,Aug_229,Aug_230,Aug_231,Aug_232,Aug_233,Aug_234,Aug_235,Aug_236,Aug_237,Aug_238,Aug_239,Aug_240,Aug_241,Aug_242,Aug_243,Sep_244,Sep_245,Sep_246,Sep_247,Sep_248,Sep_249,Sep_250,Sep_251,Sep_252,Sep_253,Sep_254,Sep_255,Sep_256,Sep_257,Sep_258,Sep_259,Sep_260,Sep_261,Sep_262,Sep_263,Sep_264,Sep_265,Sep_266,Sep_267,Sep_268,Sep_269,Sep_270,Sep_271,Sep_272,Sep_273,Oct_274,Oct_275,Oct_276,Oct_277,Oct_278,Oct_279,Oct_280,Oct_281,Oct_282,Oct_283,Oct_284,Oct_285,Oct_286,Oct_287,Oct_288,Oct_289,Oct_290,Oct_291,Oct_292,Oct_293,Oct_294,Oct_295,Oct_296,Oct_297,Oct_298,Oct_299,Oct_300,Oct_301,Oct_302,Oct_303,Oct_304,Nov_305,Nov_306,Nov_307,Nov_308,Nov_309,Nov_310,Nov_311,Nov_312,Nov_313,Nov_314,Nov_315,Nov_316,Nov_317,Nov_318,Nov_319,Nov_320,Nov_321,Nov_322,Nov_323,Nov_324,Nov_325,Nov_326,Nov_327,Nov_328,Nov_329,Nov_330,Nov_331,Nov_332,Nov_333,Nov_334,Dec_335,Dec_336,Dec_337,Dec_338,Dec_339,Dec_340,Dec_341,Dec_342,Dec_343,Dec_344,Dec_345,Dec_346,Dec_347,Dec_348,Dec_349,Dec_350,Dec_351,Dec_352,Dec_353,Dec_354,Dec_355,Dec_356,Dec_357,Dec_358,Dec_359,Dec_360,Dec_361,Dec_362,Dec_363,Dec_364,Dec_365)

Seasonal_cycle_wafh <-data.frame(
  c(1:366),
  mean_daily_wafh
)

write.csv(Seasonal_cycle_wafh, 
...
ggplot(Seasonal_cycle_wafh, aes(x=c(1:366), y=mean_daily_wafh))
+ geom_line(col="darkblue",  lwd=1) + 
...
dev.copy(png, filename="Seasonal Cycle of Rainfall of Wa (1960-1988).png",
         width=1000, height=750)
dev.off()

#sum11 <- data.frame(
#  Seasonal_cycle_wafh[
#    Seasonal_cycle_wafh[,2]==max(Seasonal_cycle_wafh[,2]),],
#  Seasonal_cycle_wafh[
#    Seasonal_cycle_wafh[,2]==min(Seasonal_cycle_wafh[,2]),]
#)
...
mean_daily_wash<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11,
                    Jan_12,Jan_13,Jan_14,Jan_15,Jan_16,Jan_17,Jan_18,Jan_19,Jan_20,Jan_21,
                    Jan_22,Jan_23,Jan_24,Jan_25,Jan_26,Jan_27,Jan_28,Jan_29,Jan_30,Jan_31,
                    Feb_32,Feb_33,Feb_34,Feb_35,Feb_36,Feb_37,Feb_38,Feb_39,Feb_40,Feb_41,
                    Feb_42,Feb_43, Feb_44,Feb_45,Feb_46,Feb_47,Feb_48,Feb_49,Feb_50,Feb_51,
...
Seasonal_cycle_wash <-data.frame(c(1:366),mean_daily_wash)
write.csv(Seasonal_cycle_wash, 
          file = "data_for_seasonal_cycle_of_Rainfall of wa(1989-2016).csv")
...
ggplot(Seasonal_cycle_wash, aes(x=c(1:366), y=mean_daily_wash)) +
  geom_line(col="darkblue", lwd=1) + 
...
dev.copy(png, filename="Seasonal Cycle of Rainfall of Wa (1960-2016).png",
         width=1000, height=750)
dev.off()



#plot(Seasonal_cycle_wafh, type="l", lwd=1, col="darkblue",
#     main="Seasonal Rainfall Cycle of Wa - 1960-2017", 
#     xlab="Days",ylab="Rainfall(mm)")

#lines(sss.1, col="blue", lwd=3)
#lines(Seasonal_cycle_wash, col="darkblue", lwd=1)
...
ggplot(Seasonal_cycle_wafh,aes(x=Seasonal_cycle_wafh[,1])) +
  geom_line(aes(y=Seasonal_cycle_wafh[,2]),lwd=.25) + 
  geom_line(aes(y=sss.1[,2], col="1960-1988"), lwd=1.25) +
  geom_line(aes(y=Seasonal_cycle_wash[,2]),lwd=.25) + 
  geom_line(aes(y=bb.1[,2],col="1989-2016"), lwd=1.25) + 
  scale_colour_manual("",
                      values = c("1960-1988"="darkblue","1989-2016"="red")) +
  xlab("Day") +
  ylab("Rainfall(mm)") + ggtitle("Seasonal Cycle of Rainfall (1960-2016)\n Wa") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Seasonal Cycle of Rianfall of Wa (1960-66-2017).png", 
         width = 1000, height = 750)
 dev.off()
...
#  geom_hline(yintercept=0) + labs(title="Index of Annual Rainfall of Wa \n (1960-1988)", x="Years", y="Index")
#dev.copy(png, filename="Index of Anual Rainfall of Wa (1960-1988).png", width=1000, height=750)
#dev.off()
...
#                                                                                                              position = "identity",size=2) + 
#  theme(legend.title = element_text(size = 2, colour = F)) + (scale_x_date(date_breaks="4 year",
#                                                                           date_labels="%Y"))  +
#  geom_hline(yintercept=0) + labs(title="Index of Annual Rainfall of Wa \n (1989-2016)", x="Years", y="Index")
#dev.copy(png, filename="Index of Annual Rainfall of Wa (1989-2016).png", width=1000, height=750)
#dev.off()
