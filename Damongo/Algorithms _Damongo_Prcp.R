...
data1 <- read.table("Damango_rr_temp.txt",header = T, 
                    sep = "\t",
                    na.strings = -99.9)
head(data1,9)
dates <- with(
...
head(Prcp)
#Deleting 1969 
#Prcp <- Prcp[!(Prcp$dates >= "1969-01-01" & Prcp$dates <= "1969-12-31"), ]
colSums(is.na(Prcp))
#Deleting NA'S
Prcp <- Prcp %>% drop_na()
...
#Delete 1969
#Prcp.df.ann <- Prcp.df.ann[!Prcp.df.ann$date == "1969-12-31", ]

#Saving data
write.csv(Prcp.df.ann,
          file="Annual Rainfall of Damongo_new (1960-2017).csv")
...
ggplot(Prcp.df.ann, aes(x=date, y=Prcp)) + geom_line(col="darkblue", lwd=2) + 
  ...
  annotate("text", x=Prcp.df.ann[Prcp.df.ann[,1] == "1990-12-31", ]$date, 
           y=500, label="P-value=0.64464", cex=5) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Annual Rainfall of Damongo (1960-2017).png", 
...
write.csv(sum1,
          file="summary Stats for ANNUAL Rainfall of Damongo.csv") 
#or
...
write.csv(sum11,
          file = "summary stats for Annual Prcp.1 (Damongo).csv")
...
write.csv(mean.mn.Prcp,
          file="Mean Monthly Rainfall of Damongo (1960-2017).csv")
...
dev.copy(png, filename="Mean Monthly Rainfall of Damongo (1960-2017).png",
         height=750, width=1000)
dev.off()

sum2 <- summary(mean.mn.Prcp)
write.csv(sum2,
          file="Summary Stats for mean monthly rainfall ofa damongo.csv")
#or
...
write.csv(sum22,
          file = "summary stats for mean mOnth Prcp.1 (Damongo).csv")




#Annual Rainy Days of Damongo
#Affected by missing cases
head(Prcp.xts)
#conversion to data frame
Prcp.xts.df <- data.frame(date=index(Prcp.xts), coredata(Prcp.xts))
names(Prcp.xts.df)[2] <- "Prcp"
Prcp.xts.df <- drop_na(Prcp.xts.df)
head(Prcp.xts.df)
...
#Deleting 1965
Ann.rndys <- Ann.rndys[!(Ann.rndys$date.ann == "1969-01-01" |
                         Ann.rndys[,1] == "1966-01-01"), ]
...
ggplot(Ann.rndys, aes(x=date.ann, y=Ann.rndys.v)) + 
  ...
  annotate("text", x=Ann.rndys[Ann.rndys$date.ann == "1993-01-01", ]$date.ann, 
           y=50, label=paste("p-value = ",c(MannKendall(Ann.rndys[,2]))$sl[1]), cex=5) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Annual Rainy Days of Damongo (1960-2017).png", 
         width=1000, height=750)
dev.off()

sum3 <- summary(Ann.rndys)
write.csv(sum3, 
          file="Summary Stats for Monthly RAINY Days grt 0.87mm of DAMONGO.csv")
#or
...
write.csv(sum33,
          file = "summary stats for ann rainy days (Damongo).csv")



# Annual Rainy Days of Damongo >= 20mm
head(Prcp.xts.df)
...
Ann.rdys20.dam <- data.frame(
  date.rdys20,
  Ann.rndys20.v
  )

#Delete 1965 and 2015
Ann.rdys20.dam <- Ann.rdys20.dam[!(Ann.rdys20.dam$date.rdys20 == "1966-01-01" | 
                                     Ann.rdys20.dam$date.rdys20 == "1967-01-01" |
                                     Ann.rdys20.dam[,1] == "1969-01-01" |
                                     Ann.rdys20.dam[,1] == "2001-01-01"), ]

write.csv(Ann.rdys20.dam, 
          file="Annual Rainy Days of Dam grt than 20mm (1960-2017).csv")

MK <- MannKendall(Ann.rdys20.dam[,2])
summary(MK)

ggplot(Ann.rdys20.dam, aes(x=date.rdys20, y=Ann.rndys20.v)) + 
  ...
  annotate("text",x=Ann.rdys20.dam[Ann.rdys20.dam$date.rdys20=="1973-01-01", ]$date.rdys20,
           y=13, label="P-value=0.84812", cex=5) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Annual Rainy Days of Damongo grt 20mm (1960-2017).png", 
         height=750, width=1000)
dev.off()

sum4 <- summary(Ann.rdys20.dam)
write.csv(sum4,
          file="Summary Stats for Annual Rainy Days when RAINFALL GRT 20mm of DAMONGO.csv")
#or
...
write.csv(sum44,
          file = "summary stats for ann rainy days grt 20 (Damongo).csv")
...
width <- length(1960:2017) #....#length of 1960-2016)
...
Dam.ann.rdys <- c(J,F,M,A,Ma,Ju,Jl,Au,S,O,N,D)
date.mn
...
write.csv(monthly.rdys, 
          file = "Monthly Rainy Days of Damongo (1960-2016).csv")

ggplot(monthly.rdys, aes(x=date.mn, y=Dam.ann.rdys)) + 
  geom_line(col="darkblue", lwd=2) + 
  labs(title="Number of Days when Rainfall >= 0.85mm (1960-2017)\n Damongo",
       x="Month", y="Rainfall(mm)") + 
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
...
dev.copy(png, filename="Monthly Rainy Days of Damongo (1960-2017)", 
         height = 500, width = 1000)
dev.off()

sum5 <- summary(monthly.rdys)
write.csv(sum5,
          file="Summary Stats for Annual Rainy Days when monthly RAINFALL grt0.87mm.csv")
#or
...
write.csv(sum55,
          file = "summary stats for monthly rainy days (Damongo).csv")
...
#creating a vector with the aboves
Dam.ann.rdys20 <- c(J.1,F.1,M.1,A.1,Ma.1,Ju.1,
...
write.csv(monthly.rdys20, 
          file = "Monthly Rainy Days of Dam grt 20 (1960-2016).csv")

ggplot(monthly.rdys20, aes(x=date.mn, y=Dam.ann.rdys20)) + 
  geom_line(col="darkblue", lwd=2) + 
...
write.csv(sum66,
          file = "summary stats for monthly rainy days grt 20mm (Damongo).csv")
...
write.csv(Prcp.df.ann.anom, 
          file="Data for Index of Annual RAINFALL of Damongo(1960-2017).csv")
...
dev.copy(png, filename="Index of Annual Rainfall of Damongo (1960-2017).png",
         height=750, width=1000)
dev.off()
...
write.csv(sum77,
          file = "summary stats for annual Prcp Anomaly (Damongo).csv")
...
mean_daily_Dam<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11, 
...
Seasonal_cycle_Dam <-data.frame(
  c(1:366),
  mean_daily_Dam
  )

write.csv(Seasonal_cycle_Dam, 
          file = "data_for_seasonal_cycle_of_Rainfall of Damongo(1960-2017).csv")
...
ggplot(Seasonal_cycle_Dam, aes(x=c(1:366), y=mean_daily_Dam)) + 
...
dev.copy(png, filename="Seasonal Rainfall Cycle of Damongo (1960-2017).png",
         height=750, width=1000)
dev.off()

sum7 <- summary(Seasonal_cycle_Dam)
write.csv(sum7,
          file="Summary Stats for Seasonal Cycle of Rainfall of Damongo.csv")
...
write.csv(Prcp_fh,file = "Annual Rainfall of DAMONGO(1960-1988).csv")

Prcp_sh <- Prcp.df.ann[(Prcp.df.ann$date >= "1989-12-31" &
                          Prcp.df.ann$date <= "2017-12-31"), ]
write.csv(Prcp_sh,file="AAnnuAl Rainfall of Dmongo(1989-2017).csv")
...
write.csv(mean.mn.Prcpfh, 
          file="Mean Monthly Rainfall of Damongo (1960-1988).csv")
...
dev.copy(png, filename="Mean Monthly Rainfall of Damongo (1960 - 1988).png", 
         height=750, width=1000)
dev.off()

sum8 <- summary(mean.mn.Prcpfh)
write.csv(sum8,
          file="Summary Stats for mean Monthly RAINFALL of Damongo (1960-1988).csv")
#or
...
write.csv(mean.mn.Prcpsh,
          "Mean Monthly Prcp of Damongo(1989-2017).csv")
...
dev.copy(png, filename="Mean Monthly Rainfall of Damongo (1989-2017.png", 
         height=750, width=1000)
dev.off()

sum8 <- summary(mean.mn.Prcpsh)
write.csv(sum8,
          file="Summary Stats for Mean monthly Rainfall of DAMONGO (1989-2017).csv")
#or
...
ggplot(mean.mn.Prcpfh,aes(x=mean.mn.Prcpfh[,1])) + 
  ...
  geom_line(aes(y=mean.mn.Prcpsh[,2],col="1989-2017"), lwd=2) +
  scale_colour_manual("",
                      values = c("1960-1988"="darkblue", "1989-2017"="red")) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b") + xlab("Month") +
  ylab(yyy) + ggtitle("Mean Monthly Rainfall (1960-2017)\n Damongo")+
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename = "Mean Monthly RAINFALL Damongo (1960-88-2017).png",
         width = 1000, height = 750)
dev.off()
...
Ann.rndys20_fh <- Ann.rdys20.dam[Ann.rdys20.dam[,1] >= "1960-01-01" &
                                        Ann.rdys20.dam[,1] <= "1988-01-01", ]

Ann.rndys20_sh <- Ann.rdys20.dam[Ann.rdys20.dam[,1] >= "1989-01-01" &
                                        Ann.rdys20.dam[,1] <= "2017-01-01", ]
...
dev.copy(png,filename="Comparing Days with Rainfall grt 20mm DAMONGO.png",
         width = 1000, height = 750)
dev.off()
...
mean_daily_Damfh<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11, Jan_12,
...
Seasonal_cycle_Damfh <-data.frame(
  c(1:366),
  mean_daily_Damfh
  )

write.csv(Seasonal_cycle_Damfh, 
          file = "data_for_seasonal_cycle_of_Rainfall of Damongo(1960-1988).csv")
...
ggplot(Seasonal_cycle_Damfh, aes(x=c(1:366), y=mean_daily_Damfh)) + 
  geom_line(col="darkblue", lwd=1) + 
...
sum8 <- summary(Seasonal_cycle_Damfh)
write.csv(sum8,
...
mean_daily_Damsh<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11, Jan_12,Jan_13,Jan_14,Jan_15,Jan_16,
...
Seasonal_cycle_Damsh <-data.frame(
  c(1:366),
  mean_daily_Damsh
  )

write.csv(Seasonal_cycle_Damsh, 
          file = "data_for_seasonal_cycle_of_Rainfall of Damongo (1989-2017).csv")
...
ggplot(Seasonal_cycle_Damsh, aes(x=c(1:366), y=mean_daily_Damsh)) + 
  geom_line(col="darkblue", lwd=1) + 
...
dev.copy(png, filename="Seasonal Cycleof Damongo (1989-2017).png",
         height=750, width=1000)
dev.off()

sum9 <- summary(Seasonal_cycle_Damsh)
write.csv(sum9,
          file = "Summary Stats for Seasonal Cycle of Rainfall (1989-2017).csv ")
...
ggplot(Seasonal_cycle_Damfh, aes(x=Seasonal_cycle_Damfh[,1])) +
  geom_line(aes(y=Seasonal_cycle_Damfh[,2]), lwd=.25) +
  geom_line(aes(y=sss.1[,2],col="1960-1988"), lwd = 1.5) +
  geom_line(aes(y=bb.1[,2], col="1989-2017"), lwd=1.25) + 
  geom_line(aes(y=Seasonal_cycle_Damsh[,2]),lwd=.25)+ 
  scale_colour_manual("",
                      values = c("1960-1988"="darkblue","1989-2017"="red")) +
...
dev.copy(png, filename="Seasonal Cycle of Rainfall of Damongo(1960-2017).png", 
         width = 1000, height = 750)
dev.off()
...
ggplot(data=Seas,
       aes(x=dates,y=Prcp)) +
  geom_line(col="darkblue", lwd=2) +
  ggtitle(" Seasonal Rainfall (1960-2017)\n Damongo") + xlab("Month") +
  ylab("Rainfall(mm)") + 
  theme(plot.title = element_text(size=18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13)) +
...
#  theme(legend.title = element_text(size = 2, colour = F)) + 
#  (scale_x_date(date_breaks="4 years",date_labels="%Y"))  +geom_hline(yintercept=0) + 
#  labs(title="Index of Annual Rainfall of Damongo \n (1960-1988)", x="Years", y="Index")
#dev.copy(png, filename="Index of Annual Rainfall of Damongo (1960-1988).png", height=750, width=1000)
#dev.off()
...
##dev.copy(png, filename="Index of Annual Rainfall ofDamongo (1989-2017).png", height=750, width=1000)
#dev.off()
