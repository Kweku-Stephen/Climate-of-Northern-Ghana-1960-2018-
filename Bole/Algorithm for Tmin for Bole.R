...
write.csv(Tmin.ann.df,
          file="Annual Minimum Temperature of Bole (1960-2017).csv")

#Removing NaNs
#Tmin.ann.df <- Tmin.ann.df %>% drop_na()

head(Tmin.ann.df)
MK <- MannKendall(Tmin[,2])
summary(MK)
...
ggplot(Tmin.ann.df, aes(x=date, y=Tmin)) +
  ...
  annotate("text", x=Tmin[Tmin$date == "1976-12-31", ]$date, y = 22.5, 
...
dev.copy(png, 
         filename="Annual Min Tmp of Bole (1060-2017).png", width=1000, height=750)
dev.off()
...
write.csv(Mn.mn.tmn,
          file="Mean Monthly Min Temp od Bole (1960-2017).csv")
...
dev.copy(png, 
         filename="Mean Monthly Minimum Temp of Bole (1960-2017).png",
...
mean_dailytmn_Bole<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11, 
...
ggplot(Seasonal_cycle_Boletmn, aes(x=c(1:366), y=mean_dailytmn_Bole)) + 
  geom_line(col="firebrick", lwd=1) + 
  labs(title="Seasonal Cycle of Minimum Temperature (1960-2017)\n Bole", 
       x="Day", y=yyy) + 
...
dev.copy(png,
         filename="Seasonal Cycle of Minimum Temp of Bole (1960-2017).png", 
         width=1000, height=750)
dev.off()
...
#Indexing of Min tmp of Bole (1960-2017)
head(Tmin.ann.df)
lng.mn.Tmin <- mean(Tmin.ann.df$Tmin, na.rm = T)
sd.Tmin <- sd(Tmin.ann.df$Tmin, na.rm = T)
Tmin.ann.df.1 <- drop_na(Tmin.ann.df)
...
write.csv(Tmin.ann.df.anom, 
          file="Index of Annual Min Tmp of Bole (1960-2017).csv")

ggplot(Tmin.ann.df.anom, aes(Tmin.ann.df.1$date, Tmin.ann.df.anom.v, ymin = 0,
...
dev.copy(png, 
         filename="Index of of Minimum Temp of Bole (1960-2017).png", 
         width=1000, height=750)
dev.off()
...
write.csv(Tmin_fh, 
          file="Annual Min.Tmp of Bole (1960-88).csv")
...
ggplot(Tmin_fh, aes(x=Tmin_fh$date, y=Tmin_fh$Tmin)) + 
  ...
  annotate("text", x=Tmin_fh[Tmin_fh$date == "1967-12-31", ]$date, 
           y = 21.0, label = "P-value = 0.00016022", cex = 5) + 
...
dev.copy(png,
         filename="Annual Min Tmp od Bole (1960-1988).png", 
         width=1000, height=750)
dev.off()
...
write.csv(mean.mn.Tminfh,
          file = "Mean Monthly Tmin of Bole (1960-1988).csv")
...
dev.copy(png,
         filename="Mean Monthly Min Tmp od Bole (1960-1988).png", 
         width=1000, height=750)
dev.off()
...
write.csv(Tmin_sh,
          file="Annual Tmin of Bole (1989-2017).csv")
...
ggplot(Tmin_sh, aes(x=Tmin_sh$date, y=Tmin_sh$Tmin)) + 
  ...
  labs(title='Minimum Temperature (1989-2017)\n Bole', 
       x="Year", y=yyy) + 
  scale_x_date(date_breaks = "3 years", date_labels = "%Y") + 
  annotate("text", x=Tmin_sh[Tmin_sh[,1] == "1995-12-31", ]$date,
           y = 22.5, label="P-value = 2.8729e-05", cex = 5) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, 
         filename="Annual Minimum Tmp od Bole (1989-2017).png", 
         width=1000, height=750)
dev.off()
...
write.csv(mean.mn.Tminsh,
          file = "Mean Monthly Min Tmp of Bole (1989-2017).csv")
...
dev.copy(png, filename="Mean Monthly Min Tmp od Bole (1989-2017).png", 
         width=1000, height=750)
dev.off()
...
#plot(mean.mn.Tminfh, type="l", lwd=3, col="darkblue",ylim=c(15,25) ,
#     main="Mean Monthly Min Tmp of Bole - 1960-2017", xlab="Months",ylab=yyy)
...
ggplot(mean.mn.Tminfh,aes(x=mean.mn.Tminfh[,1])) + 
  ...
  xlab("Month") + ylab(yyy) + scale_x_date(date_breaks = "1 month",
                     date_labels = "%b") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename = "Mean Monthly Min Tmp of Bole (1960-88-2017).png",
         width = 1000, height = 750)
dev.off()



#Min Tmp Change between two halves
...
mean_dailytmn_Bolefh<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,
                         ...
                         Apr_111,Apr_112,Apr_113,Apr_114,Apr_115,Apr_116,Apr_117,Apr_118,
...
Seasonal_cycle_Boletmnfh <-data.frame(
  c(1:366),
  mean_dailytmn_Bolefh
  )

write.csv(Seasonal_cycle_Boletmnfh,
          file = "data_for_seasonal_cycle_of_Min tmp of Bole(1960-1988).csv")
...
ggplot(Seasonal_cycle_Boletmnfh, aes(x=c(1:366), y=mean_dailytmn_Bolefh)) +
  geom_line(col="darkblue", lwd=1) + 
  labs(title="Seasonal Cycle of Minimum Temperature (1960-1988)\n Bole",
       x="Day", y=yyy) + 
  geom_line(data = gg.1, aes(x=x, y=y), col="red", lwd=1) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, 
         filename="Seasonal Cycle of Min Tmp od Bole (1960-1988).png",
         width=1000, height=750)
dev.off()
...
mean_dailytmn_Bolesh<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,
                         Jan_11, Jan_12,Jan_13,Jan_14,Jan_15,Jan_16,Jan_17,Jan_18,Jan_19,
                         ...
                         Feb_40,Feb_41,Feb_42,Feb_43, Feb_44,Feb_45,Feb_46,Feb_47,Feb_48,
                         Feb_49,Feb_50,Feb_51,Feb_52,Feb_53,Feb_54,Feb_55,Feb_56,Feb_57,Feb_58,
...
Seasonal_cycle_Boletmnsh <-data.frame(
  c(1:366),
  mean_dailytmn_Bolesh
  )

write.csv(Seasonal_cycle_Boletmnsh, 
          file = "data_for_seasonal_cycle_of_Min tmp of Bole(1989-2017).csv")
...
ggplot(Seasonal_cycle_Boletmnsh, aes(x=c(1:366), y=mean_dailytmn_Bolesh)) +
  geom_line(col="darkblue", lwd=1) + 
  labs(title="Seasonal Cycle of Minimum Temperature (1989-2017)\n Bole",
       x="Day", y=yyy) + 
  geom_line(data = ee.1, aes(x=x, y=y), col="red", lwd=1) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Seasonal Cycle of Min Tmp of Bole (1989-2017).png",
         width=1000, height=750)
dev.off()



#plot(Seasonal_cycle_Boletmnfh, main="Seasonal Min.Tmp Cycle of Bole - 1960-2017", xlab="Days", ylab=yyy, type="l", col="firebrick", ylim=c(14,26))
#lines(gg.1, col="darkblue", lwd=3)
#lines(Seasonal_cycle_Boletmnsh, col="firebrick", type="l")
#lines(ee.1, col="red",lwd=3)
#legend("topright", col=c("darkblue", "red"), legend=c("1960-1988", "1989-2017"), bty='n', lwd=3, lty=1)


ggplot(Seasonal_cycle_Boletmnfh, aes(x=Seasonal_cycle_Boletmnfh[,1])) + 
  geom_line(aes(y=Seasonal_cycle_Boletmnfh[,2]), lwd = .25)+
  geom_line(aes(y= gg.1[,2],col = "1960-1988"), lwd = 1.25) + 
  geom_line(aes(y=Seasonal_cycle_Boletmnsh[,2]), lwd = .25) + 
  geom_line(aes(y=ee.1[,2],col = "1989-2017"), lwd = 1.25) +
  scale_colour_manual("",values = c("1960-1988"="darkblue","1989-2017"="red")) +
  labs(title = "Seasonal Cycle of Minimum Temperature (1960-2017)\n Bole", 
       x = 'Day',y=yyy) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
...
dev.copy(png, filename="Seasonal Min Tmp Cycle of Bole (1960-88-2017).png",
         width=1000, height=750)
dev.off()
...
#first half
#head(Tmin_fh)
#lng.mn.Tminfh <- mean(Tmin_fh$Tmin, na.rm = T)
#sd.Tminfh <- sd(Tmin_fh$Tmin, na.rm = T)
#Tmin_fh.1 <- drop_na(Tmin_fh)
#Tmin_fh_anom.v <- (Tmin_fh.1$Tmin - lng.mn.Tminfh)/sd.Tminfh
#Tmin_fh_anom <- data.frame(Tmin_fh.1$date, Tmin_fh_anom.v)
#ggplot(Tmin_fh_anom, aes(Tmin_fh.1$date, Tmin_fh_anom.v, ymin = 0,ymax = Tmin_fh_anom.v)) + 
...
#  geom_hline(yintercept=0) + labs(title="Index of Annual Minimum Tmp of Bole \n (1960-1988)", x="Years", y="Index")
#dev.copy(png, filename="Index of Min Tmp od Bole (1960-1988).png", width=1000, height=750)
#dev.off()


#Second half
#head(Tmin_sh)
#lng.mn.Tminsh <- mean(Tmin_sh$Tmin, na.rm = T)
#sd.Tminsh <- sd(Tmin_sh$Tmin, na.rm = T)
#Tmin_sh.1 <- drop_na(Tmin_sh)
...
#
#dev.copy(png, filename="Seasonal Cycle of Min Tmp of Bole (1989-2017).png", width=1000, height=750)
#dev.off()
