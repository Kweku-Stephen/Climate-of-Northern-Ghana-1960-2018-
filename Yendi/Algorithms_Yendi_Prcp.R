...
setwd("F:/R_PROJECTS/Yendi")
getwd()

data1 <- read.table(
  "Yendi_rr_temp.txt", sep = "\t",
  header = T, 
  na.strings = -99.9
)
...
# Prcp
Prcp <- data.frame(data1$dates, data1$Prcp)
names(Prcp)[1:2] <- c("date","Prcp")
head(Prcp)
#Prcp <- Prcp %>% drop_na()

#Annual Prcp
Prcp.xts <- xts(Prcp$Prcp, Prcp$date)
...
write.csv(Prcp.df.ann, 
          file="Annual Rainfall of Yendi (1960-2017).csv")
...
ggplot(Prcp.df.ann, aes(x=date, y=Prcp)) + geom_line(col="darkblue", lwd=2) +
  ...
  scale_x_date(date_breaks = "4 years", date_labels = "%Y") + annotate("text", 
          x=Prcp.df.ann[Prcp.df.ann$date == "1979-12-31", ]$date, y = 1625, cex = 5,
          label = "P-valaue = 0.40553") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

MK <- MannKendall(Prcp.df.ann$Prcp)
summary(MK)

dev.copy(png, filename = "Annual Rainfall of Yendi(1960-2017.png", 
         width =1000,height =750)
dev.off()

sum1 <- summary(Prcp.df.ann)
write.csv(Prcp.df.ann,
          file="Summary Statistics for Annual Prcp Total (1960-2017).csv")
#or
...
write.csv(sum11,
          file = "summary stats for Annual Prcp.1 (Yendi).csv")
...
write.csv(mean.mn.Prcp, 
          file="Mean Monthly Rainfall of Yendi (1960-2017).csv")
...
dev.copy(png, filename = "Mean Monthly Rainfall of Yendi (1960-2917).png",
         width = 1000, height = 750)
dev.off()

sum2 <- summary(mean.mn.Prcp)
write.csv(mean.mn.Prcp,
          file = "Summary statistics for mean monthly Rainfalll of Yendi(1960-2017).csv")
...
write.csv(sum22,
          file = "summary stats for mean mOnth Prcp.1 (Yendi).csv")



#Annual Rainy Days of Yendi
...
b.60.1 <- nrow(subset(Prcp.xts.df, format.Date(Prcp.xts.df$date, "%y")=="75")
               [subset(Prcp.xts.df, format.Date(Prcp.xts.df$date, 
                                                "%y")=="75")$Prcp >= 0.85, ])
# to
...
Ann.rndys.v <- c(b.60.1, b.61.1,b.62.1,b.63.1,b.64.1,b.65.1,b.66.1,b.67.1,b.68.1,b.69.1,
                 b.70.1,b.71.1,b.72.1,b.73.1,b.74.1,b.75.1, b.76.1, b.77.1, b.78.1, b.79.1,
                 b.80.1, b.81.1, b.82.1, b.83.1, b.84.1, b.85.1, b.86.1, b.87.1, b.88.1, 
                 b.89.1, b.90.1, b.91.1, b.92.1, b.93.1, b.94.1, b.95.1, b.96.1, b.97.1,
                 b.98.1, b.99.1, b.00.1, b.01.1, b.02.1, b.03.1, b.04.1, b.05.1, b.06.1, 
                 b.07.1, b.08.1, b.09.1, b.10.1, b.11.1, b.12.1, b.13.1, b.14.1, b.15.1,
                 b.16.1, b.17.1)

date.ann <- seq(as.Date
                ("1960-01-01"), by="years", 
...
#Ann.rndys <- Ann.rndys[!(Ann.rndys$date.ann >= "1986-01-01" & 
                        #Ann.rndys$date.ann <= "2001-01-01"), ]
...
write.csv(Ann.rndys, 
          file="Annual Rainy Days of Yendi (1960-2017).csv")
...
ggplot(Ann.rndys, aes(x=date.ann, y=Ann.rndys.v)) + 
  ...
  labs(title="Number of Days when Rainfall >= 0.85mm (1960-2017)\n Yendi", 
       x="Year",y="Rainy Days") + 
  geom_smooth(method = lm, col="red") + annotate("text", 
            x = Ann.rndys[Ann.rndys[,1] == "1977-01-01", ]$date.ann, y = 95, cex = 5,
            label= "P-value = 0.25889") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename = "Annual Rainy Days of  Yendi (1960-2017).png",
         width = 1000, height = 750)
dev.off()

sum3 <- summary(Ann.rndys)
write.csv(sum3,
          file="summary stats of Annual Rainy Days of Yendi with Prcp grt 1mm(1960-2017).csv")
...
write.csv(sum33,
          file = "summary stats for ann rainy days (Yendi).csv")
...
Prcp.ann.rdys20 <- Prcp.xts.df[
  Prcp.xts.df$Prcp >= 20, 
  ]

b.60.2 <- nrow(
  subset(Prcp.ann.rdys20, format.Date(Prcp.ann.rdys20$date, "%y")=="60"
         )
  )

#to
b.17.2 <- nrow(
  subset(Prcp.ann.rdys20, format.Date(Prcp.ann.rdys20$date, "%y")=="17"
         )
  )  
...
date.rdys20 <- seq(as.Date("1960-01-01"), by = "years", 
                   length.out = length(Ann.rndys20.v))

Ann.rdys20.Yendi <- data.frame(
  date.rdys20, 
  Ann.rndys20.v
)

#Deleting the years period 1986 - 2001
#Ann.rdys20.Bolga <- Ann.rdys20.Bolga[!(Ann.rdys20.Bolga$date.rdys20 >= "1986-01-01" & 
#                                       Ann.rdys20.Bolga$date.rdys20 <= "2001-01-01"), ]
...
write.csv(Ann.rdys20.Yendi, 
          file="Annual Rainy Days of Yendi grt than 20mm (1960-2017).csv")

ggplot(Ann.rdys20.Yendi, aes(x=date.rdys20, y=Ann.rndys20.v)) + 
  geom_line(col="darkblue", lwd=2) + geom_smooth(method = lm, col="red") + 
  scale_x_date(date_breaks = "4 years", date_labels = "%Y") + 
  ggtitle("Number of Days when Rainfall >= 20mm (1960-2017)\n Yendi") +
  xlab("Year") + ylab("Rainfall(mm)") + annotate("text", 
         x = Ann.rdys20.Yendi[Ann.rdys20.Yendi[,1] == "1969-01-01", ]$date.rdys20, 
         y = 30, cex = 5, label= "P-value = 1") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename = "Annual Rainy Days of Yendi grt than 20mm.png", 
         width = 1000,height = 750)
dev.off()

sum4 <- summary(Ann.rdys20.Yendi)
write.csv(sum4,
          file="summary stats of Annual Rainy Days of Yendi with Prcp grt 20mm(1960-2017).csv")

#or
sum44 <- Ann.rdys20.Yendi[Ann.rdys20.Yendi[,2] == max(Ann.rdys20.Yendi[,2]), ]
sum44[3:5,c(1,2)] <-  Ann.rdys20.Yendi[Ann.rdys20.Yendi[,2] == min(Ann.rdys20.Yendi[,2]), ]
...
write.csv(sum44,
          file = "summary stats for ann rainy days grt 20 (Yendi).csv")
...
#"subsetting individual months out of 
#the Prcp.mn.rdys data and identifying number of rainy days"
...
Yendi.ann.rdys <- c(J,F,M,A,Ma,Ju,Jl,Au,S,O,N,D)
date.mn
...
write.csv(monthly.rdys, 
          file = "Monthly Rainy Days of Yendi (1960-2017).csv")

ggplot(monthly.rdys, aes(x=date.mn, y=Yendi.ann.rdys)) + 
  geom_line(col="darkblue", lwd=2) + 
  labs(title="Number of Dyas when Rainfall >= 0.85mm (1960-2017)\n Yendi",
...
dev.copy(png, filename="Monthly Rainy Days of Yendi (1960-2017).png",
         height = 750, width = 1000)
dev.off()

sum5 <- summary(monthly.rdys)
write.csv(sum5,
          file="summayr stats of Monthly Rainy Days of Yendi with Prcp grt 1mm(1960-2017).csv")
#or
...
write.csv(sum55,
          file = "summary stats for monthly rainy days (Yendi).csv")



#Monthly Rainy Days >= 20mm
head(Prcp)
...
#creating a vector with the aboves
Yendi.ann.rdys20 <- c(J.1,F.1,M.1,A.1,Ma.1,Ju.1,
...
write.csv(monthly.rdys20, 
          file = "Monthly Rainy Days of Yendi grt 20 (1960-2017).csv")

ggplot(monthly.rdys20, aes(x=date.mn, y=Yendi.ann.rdys20)) + 
  geom_line(col="darkblue", lwd=2) + 
...
dev.copy(png, filename="Monthly Rainy Days of Yendi (1960-2017)", 
         height = 750, width = 1000)
dev.off()

sum6 <- summary(monthly.rdys20)
write.csv(sum6,
          file="summary stats of Monthly Rainy Days of Yendi with Prcp grt 20mm(1960-2017).csv")
#or
...
write.csv(sum66,
          file = "summary stats for monthly rainy days grt 20mm (Yendi).csv")
...
write.csv(Prcp.df.ann.anom, 
          file = "Data for Index of Annual Rainfall of Yendi (1960-1988).csv")
...
dev.copy(png, filename="Index of Annual Rainfall of Yendi (1960-2017)", 
         height = 750, width = 1000)
dev.off()

sum7 <- summary(Prcp.df.ann.anom)
write.csv(sum7,
          file="summary stats of Anaomaly of Annual Rainfall Total of Yendi(1960-2017).csv")
...
write.csv(sum77,
          file = "summary stats for annual Prcp Anomaly (Yendi).csv")
...
mean_daily_Yendi<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11,
...
ggplot(Seasonal_cycle_Yendi, aes(x=c(1:366), y=mean_daily_Yendi)) + 
...
dev.copy(png, filename="Seasonal Cycle of Annual Rainfall of Yendi (1960-2017)", 
         height = 750, width = 1000)
dev.off()


sum8 <- summary(Seasonal_cycle_Yendi)
write.csv(sum8,
          file="summary stats of RAINFALL tOTAL Cycle(1960-2017).csv")
...
write.csv(Prcp_sh,"Annual PRcp of Yendi(1989-2017).csv")


#Annual Rainfall first half (1960-1988)
#Prcp_fh
#write.csv(Prcp_fh, file="Annual Prcp of Yendi (1960-88).csv")
#ggplot(Prcp_fh, aes(x=Prcp_fh$date, y=Prcp_fh$Prcp)) + 
#  geom_line(col="darkblue", lwd=2) + geom_smooth(method = lm, col="red") + 
#  labs(title='ANNUAL Rainfall of Yendi (1960-1988)', x="Years", y="Rainfall(mm)") 
...
write.csv(mean.mn.Prcpfh, 
          file="Mean Monthly Yendi of Damongo (1960-1988).csv")
...
dev.copy(png, filename="Mean Monthly Rainfall of Yendi (1960-1988)", 
         height = 750, width = 1000)
dev.off()

sum9 <- summary(mean.mn.Prcpfh)
write.csv(sum9,
               file="summary stats of Rainfall Total of Yendi(1960-1988).csv")
#or
...
#head(Prcp_sh)
#write.csv(Prcp_sh, file="Annual Prcp of Yendi (1989-2017).csv")
#ggplot(Prcp_sh, aes(x=Prcp_sh$date, y=Prcp_sh$Prcp)) + 
#geom_line(col="darkblue", lwd=2) + geom_smooth(method = lm, col="red") + 
#  labs(title='ANNUAL Rainfall of Bolga (1997-2017)', x="Years", y="Rainfall(mm)")
...
write.csv(mean.mn.Prcpsh, 
          file="Mean Monthly Rainfall of Yendi (1989-2017).csv")
...
dev.copy(png, filename="Mean Monthly Rainfall of Yendi (1989-2017)", 
         height = 750, width = 1000)
dev.off()

sum10 <- summary(Seasonal_cycle_Yendi)
write.csv(sum10,
            file="summary stats for mean mOnthly Rainfall of yENDI (1960-1988).csv")
#or
...
#plot(mean.mn.Prcpfh, type="l",ylim=c(0,300) ,
#     main="Mean Monthly Rainfall of Yendi- 1960-2017", 
#     lwd=3, col="darkblue", xlab="Months",ylab="Rainfall(mm)")#

#lines(mean.mn.Prcpsh, lwd=3, col="red", type="l")
...
#  ylab("Rainfall(mm)") + ggtitle("Mean Monthly Rainfall of Yendi\n (1960-2017)") 

#dev.copy(png, filename = "Mean Monthly Rainfall of Yendi (1960-88-2017).png",
#         width = 1000, height = 750)
#dev.off()



#Super imposing both plots
...
dev.copy(png, filename = "Mean Monthly Rainfall of Yendi (1960-88-2016).png",
         width = 1000, height = 750)
dev.off()
...
Ann.rndys20_fh <- Ann.rdys20.Yendi[Ann.rdys20.Yendi[,1] >= "1960-01-01" &
                                     Ann.rdys20.Yendi[,1] <= "1988-01-01", ]

Ann.rndys20_sh <- Ann.rdys20.Yendi[Ann.rdys20.Yendi[,1] >= "1989-01-01" &
                                     Ann.rdys20.Yendi[,1] <= "2017-01-01", ]
...
dev.copy(png,filename="Comparing Days with Rainfall grt 20mm Yendi.png",
         width = 1000, height = 750)
dev.off()
...
head(Prcp.mn.rdys20) 
Prcp.mn.rdysfh20 <- Prcp.mn.rdys20[Prcp.mn.rdys20[,1] >= "1960-12-31" &
                                      Prcp.mn.rdys20[,1] <= "1988-12-31", ] %>% drop_na()

Prcp.mn.rdysh20 <- Prcp.mn.rdys20[Prcp.mn.rdys20[,1] >= "1989-12-31" &
...
mean_daily_Yendifh<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,
                       ...
                       Jan_29,Jan_30,Jan_31,Feb_32,Feb_33,Feb_34,Feb_35,Feb_36,Feb_37,Feb_38,
                       Feb_39,Feb_40,Feb_41,Feb_42,Feb_43, Feb_44,Feb_45,Feb_46,Feb_47,
...
Seasonal_cycle_Yendifh <-data.frame(
  c(1:366),
  mean_daily_Yendifh
)

write.csv(Seasonal_cycle_Yendifh, 
          file = "data_for_seasonal_cycle_of_Rainfall of Yendi(1960-1988).csv")
...
ggplot(Seasonal_cycle_Yendifh, aes(x=c(1:366), y=mean_daily_Yendifh)) + 
  geom_line(col="darkblue", lwd=1) + 
...
dev.copy(png, filename="Seasonal Cycle of Annual Rainfall  of Yendi (1960-1988)", 
         height = 750, width = 1000)
dev.off()

sum11 <- summary(Seasonal_cycle_Yendifh)
write.csv(sum11,
             file="summary stats of RAINFALL tOTAL Cycle(1960-1988).csv")
...
mean_daily_Yendish<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,
                       Jan_11, Jan_12,Jan_13,Jan_14,Jan_15,Jan_16,Jan_17,Jan_18,Jan_19,
                       Jan_20,Jan_21,Jan_22,Jan_23,Jan_24,Jan_25,Jan_26,Jan_27,Jan_28,
                       ...
                       Mar_73,Mar_74,Mar_75,Mar_76,Mar_77,Mar_78,Mar_79,Mar_80,Mar_81,Mar_82,
                       Mar_83,Mar_84,Mar_85,Mar_86,Mar_87,Mar_88,Mar_89,Mar_90,Apr_91,Apr_92,Apr_93,Apr_94,Apr_95,Apr_96,Apr_97,Apr_98,Apr_99,Apr_100,Apr_101,Apr_102,Apr_103,Apr_104,Apr_105,Apr_106,Apr_107,Apr_108,Apr_109,Apr_110,Apr_111,Apr_112,Apr_113,Apr_114,Apr_115,Apr_116,Apr_117,Apr_118,Apr_119,Apr_120,May_121,May_122,May_123,May_124,May_125,May_126,May_127,May_128,May_129,May_130,May_131,May_132,May_133,May_134,May_135,May_136,May_137,May_138,May_139,May_140,May_141,May_142,May_143,May_144,May_145,May_146,May_147,May_148,May_149,May_150,May_151,Jun_152,Jun_153,Jun_154,Jun_155,Jun_156,Jun_157,Jun_158,Jun_159,Jun_160,Jun_161,Jun_162,Jun_163,Jun_164,Jun_165,Jun_166,Jun_167,Jun_168,Jun_169,Jun_170,Jun_171,Jun_172,Jun_173,Jun_174,Jun_175,Jun_176,Jun_177,Jun_178,Jun_179,Jun_180,Jun_181,Jul_182,Jul_183,Jul_184,Jul_185,Jul_186,Jul_187,Jul_188,Jul_189,Jul_190,Jul_191,Jul_192,Jul_193,Jul_194,Jul_195,Jul_196,Jul_197,Jul_198,Jul_199,Jul_200,Jul_201,Jul_201,Jul_203,Jul_204,Jul_205,Jul_206,Jul_207,Jul_208,Jul_209,Jul_210,Jul_211,Jul_212,Aug_213,Aug_214,Aug_215,Aug_216,Aug_217,Aug_218,Aug_219,Aug_220,Aug_221,Aug_222,Aug_223,Aug_224,Aug_225,Aug_226,Aug_227,Aug_228,Aug_229,Aug_230,Aug_231,Aug_232,Aug_233,Aug_234,Aug_235,Aug_236,Aug_237,Aug_238,Aug_239,Aug_240,Aug_241,Aug_242,Aug_243,Sep_244,Sep_245,Sep_246,Sep_247,Sep_248,Sep_249,Sep_250,Sep_251,Sep_252,Sep_253,Sep_254,Sep_255,Sep_256,Sep_257,Sep_258,Sep_259,Sep_260,Sep_261,Sep_262,Sep_263,Sep_264,Sep_265,Sep_266,Sep_267,Sep_268,Sep_269,Sep_270,Sep_271,Sep_272,Sep_273,Oct_274,Oct_275,Oct_276,Oct_277,Oct_278,Oct_279,Oct_280,Oct_281,Oct_282,Oct_283,Oct_284,Oct_285,Oct_286,Oct_287,Oct_288,Oct_289,Oct_290,Oct_291,Oct_292,Oct_293,Oct_294,Oct_295,Oct_296,Oct_297,Oct_298,Oct_299,Oct_300,Oct_301,Oct_302,Oct_303,Oct_304,Nov_305,Nov_306,Nov_307,Nov_308,Nov_309,Nov_310,Nov_311,Nov_312,Nov_313,Nov_314,Nov_315,Nov_316,Nov_317,Nov_318,Nov_319,Nov_320,Nov_321,Nov_322,Nov_323,Nov_324,Nov_325,Nov_326,Nov_327,Nov_328,Nov_329,Nov_330,Nov_331,Nov_332,Nov_333,Nov_334,Dec_335,Dec_336,Dec_337,Dec_338,Dec_339,Dec_340,Dec_341,Dec_342,Dec_343,Dec_344,Dec_345,Dec_346,Dec_347,Dec_348,Dec_349,Dec_350,Dec_351,Dec_352,Dec_353,Dec_354,Dec_355,Dec_356,Dec_357,Dec_358,Dec_359,Dec_360,Dec_361,Dec_362,Dec_363,Dec_364,Dec_365)

Seasonal_cycle_Yendish <-data.frame(c(1:366),mean_daily_Yendish)
write.csv(Seasonal_cycle_Yendish, 
          file = "data_for_seasonal_cycle_of_Rainfall of Yendi(1989-2017).csv")
y <- ksmooth(c(1:366), mean_daily_Yendish, bandwidth = 15)
y.1 <- data.frame(y) # converting to a dataframe

ggplot(Seasonal_cycle_Yendish, aes(x=c(1:366), y=mean_daily_Yendish)) +
  geom_line(col="darkblue", lwd=1) + 
  labs(title="Seasonal Cycle of Rainfall (1989-2017)\n Yendi", 
       x="Day", y="Rainfall(mm)") + 
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))geom_line(data = y.1, aes(x=x, y=y), col="red", lwd=1) +
  

dev.copy(png, filename="Seasonal Cycle of Annual Rainfall of Yendi (1989-2017)", 
         height = 750, width = 1000)
dev.off()

sum12 <- summary(Seasonal_cycle_Yendish)
write.csv(sum12,
          file="summary stats of RAINFALL tOTAL Cycle(1989-2017).csv")



#plot(Seasonal_cycle_Yendifh,type="l",lwd=1, col="darkblue",
#     main="Seasonal Rainfall Cycle of Yendi- 1960-2017", 
#     xlab="Days",ylab="Rainfall(mm)")

#lines(z.1, lwd=3, col='blue')

#lines(Seasonal_cycle_Yendish, type="l",
#      col="darkblue")
...
#ggplot(Seasonal_cycle_Yendifh, aes(x=Seasonal_cycle_Yendifh[,1], 
#                                  y=Seasonal_cycle_Yendifh[,2])) + 
#  geom_line(col="darkblue", lwd=.25) + geom_line(data=z.1, mapping = aes(x=x,y=y),
#                                                 col="darkgreen", lwd=1.25) +
#  geom_line(data=Seasonal_cycle_Yendish, mapping = aes(Seasonal_cycle_Yendish[,1],
#                                                       y=Seasonal_cycle_Yendish[,2]),
#            col="darkblue", lwd=.25) + geom_line(data=y.1, mapping = aes(x=x,y=y),
#                                                col="red", lwd=1.25) + 
#  labs(title = "Rainfall Seasonal Cycle of Yendi \n (1960-2017", 
#       x="Days",y="Rainfall(mm)")

#dev.copy(png, filename="Seasonal Ccle of Rainfall of Yendi (1960-88-2017).png",
#         width= 1000, height = 750)
#dev.off()
...
#Super imposing
ggplot(data=Seasonal_cycle_Yendifh,aes(x=Seasonal_cycle_Yendifh[,1]),lwd=.25) +
  geom_line(aes(y=Seasonal_cycle_Yendifh[,2]), lwd=.25) +
  geom_line(aes(y = z.1[,2], colour = "1960-1988"), lwd=1.5) +
  geom_line(aes(y = Seasonal_cycle_Yendish[,2]), lwd=.25) +
  geom_line(aes(y=y.1[,2], colour = "1989-2017"), lwd=1.5) +
  scale_colour_manual("", 
                      values = c("1960-1988"="darkgreen", "1989-2017"="red")) +
  xlab("Days") + ylab(yyy) + 
  ggtitle("Seasonal Cycle of Rainfall Total (1960-2017)\n Yendi") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Seasonal Cycle of Min Tmp of Yendi(1960-88-2017).png", 
         width = 1000, height = 750)
dev.off()
...
head(Prcp)
Ap <- mean(subset(Prcp,format.Date(date, "%m")=="04")$Prcp, na.rm=T)
...
Seas<- data.frame(
  ...
  Prcp=c(Ap2,Ma2,Ju2,Jl2,Au2,Se2,Oc2)
)
...
#  geom_linerange(data = Prcp_fh_anom, aes(colour = ifelse(Prcp_fh_anom.v >0, 
#        "Positive", "Negative")),stat = "identity", position = "identity",size=2) + 
#  theme(legend.title = element_text(size = 2, colour = F)) +
#  (scale_x_date(date_breaks="4 years",date_labels="%Y"))  +
#  geom_hline(yintercept=0) + 
#  labs(title="Index of Annual Rainfall of Yendi \n (1960-1988)", x="Years", y="Index")

#dev.copy(png, filename="Index of Annual Raainfall of Yendi (1960-1988)", 
#         height = 750, width = 1000)
#dev.off()
...
#ggplot(Prcp_sh_anom, aes(Prcp_sh$date, Prcp_sh_anom.v, ymin = 0,
#                         ymax = Prcp_sh_anom.v)) + 
#  geom_linerange(data = Prcp_sh_anom, aes(colour = ifelse(Prcp_sh_anom.v >0, 
#            "Positive", "Negative")),stat = "identity",position = "identity",size=2) + 
#  theme(legend.title = element_text(size = 2, colour = F)) + 
#  (scale_x_date(date_breaks="4 years",date_labels="%Y"))  +
#  geom_hline(yintercept=0) + 
#  labs(title="Index of Annual Rainfall of Yendi \n (1988-2017)", x="Years", y="Index")

#dev.copy(png, filename="Index of Annual  rainfall of Yendi (1989-2017)", 
#         height = 750, width = 1000)
#dev.off()
