...
data1 <- read.table("Walewale_rr.txt",header = T, 
                    sep = "\t", na.strings = -99.9)
...
write.csv(Prcp.df.ann, 
          file="Annual Rainfall of Walewale (1976-2017).csv")
MK <-MannKendall(Prcp.df.ann[,2])
summary(MK)

ggplot(Prcp.df.ann, aes(x=date, y=Prcp)) + geom_line(col="darkblue", lwd=2) + 
  geom_smooth(method = lm, col="red") + 
  labs(title="Rainfall Total (1976-2017)\n Walewale",
       x="Year", y="Rainfall(mm)") + 
  scale_x_date(date_breaks = "4 years", date_labels = "%Y") + 
  annotate("text", x=Prcp.df.ann[Prcp.df.ann[,1] == "1990-12-31", ]$date,
           y=1400, label="P-value=0.18798", cex = 5) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Annual Rainfall of Walewale (1976-2017).png", 
         height=750, width=1000)
dev.off()

sum1 <- summary(Prcp.df.ann)
write.csv(sum1,
          file="summary stats for Annual Rainfall of Walewale.csv")
...
write.csv(sum11,
          file = "summary stats for Annual Prcp.1 (Walewale).csv")
...
write.csv(mean.mn.Prcp, 
          file="Mean Monthly Rainfall of Walewale (1976-2017).csv")

ggplot(mean.mn.Prcp, aes(x=date.mn, y=mean.mn.Prcp.v)) + 
  geom_line(col="darkblue", lwd=2) + scale_x_date(date_breaks = "1 month",
                                                  date_labels = "%b") + 
  labs(title="Mean Monthly Rainfall (1976-2017)\n Walewale", 
       x="Month",y="Rainfall(mm)") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Mean Monthly Rainfall of Walewale (1976-2017).png", 
         height=750, width=1000)
dev.off()

sum2 <- summary(mean.mn.Prcp)
write.csv(sum2,
          file="summary Stats for mean mOnthly Rainfall of Walewale.csv")
...
b.76.1 <- nrow(subset(Prcp.xts.df, format.Date(Prcp.xts.df$date, "%y")=="60")
...
Ann.rndys.v <- c(b.76.1, b.77.1, b.78.1, b.79.1, b.80.1, b.81.1, b.82.1, b.83.1, 
...
write.csv(Ann.rndys, 
          file="Annual Rainy Days of Walewale (1976-2017).csv")
...
ggplot(Ann.rndys, aes(x=date.ann, y=Ann.rndys.v)) + 
  ...
  scale_x_date(date_breaks = "4 years", date_labels = "%Y") + 
  labs(title="Number of Days when Rainfall >= 0.85 (1976-2017)\n Walewale",
       x="Year",y="Rainy Days") + 
  geom_smooth(method = lm, col="red") + 
  annotate("text", x=Ann.rndys[Ann.rndys$date.ann == "1982-01-01", ]$date.ann,
           y=75, label="P-value=0.00034428", cex = 5) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Annual Rainy Days of Walewale (1976-2017).png", 
         width=1000, height=750)
dev.off()

sum3 <- summary(Ann.rndys)
write.csv(sum3,
          file = "summmary stats for Annual Rainy Days of Walewale(1976-2017).csv")
...
write.csv(sum33,
          file = "summary stats for ann rainy days (Walewale).csv")
...
#vectorization
Ann.rndys20.v <- c( b.76.2, b.77.2, b.78.2, b.79.2, b.80.2,
...
Ann.rdys20.Walewale <- data.frame(date.rdys20, Ann.rndys20.v)

#Delete 1999
Ann.rdys20.Walewale <- Ann.rdys20.Vea[!Ann.rdys20.Vea$date.rdys20 == "1999-01-01", ]
write.csv(Ann.rdys20.Walewale,
          file="Annual Rainy Days of Walewale grt than 20mm (1976-2017).csv")

MK <- MannKendall(Ann.rdys20.Walewale[,2])
summary(MK)

ggplot(Ann.rdys20.Walewale, aes(x=date.rdys20, y=Ann.rndys20.v)) + 
  ...
  annotate("text", x=Ann.rdys20.Walewale[Ann.rdys20.Walewale[,1] == "1988-01-01", ]$
             date.rdys20, y=30, label="P-value=0.34162",cex=5) +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Annual Rainy Days of Walewale grt than 20mm (1972-2017).png",
         height=750, width=1000)
dev.off()

sum4 <- summary(Ann.rdys20.Walewale)
write.csv(sum4,
          file = "Summary Stats for Annual Riany Days of Walewale grt 20mm.csv")

sum44 <- data.frame(Ann.rdys20.Walewale[Ann.rdys20.Walewale[,2] == max(Ann.rdys20.Walewale[,2]), ],
                    Ann.rdys20.Walewale[Ann.rdys20.Walewale[,2] == min(Ann.rdys20.Walewale[,2]), ])
...
write.csv(sum44,
          file = "summary stats for ann rainy days grt 20 (Walewale).csv")
...
width <- length(1976:2017) #....#length of 1960-2016)
...
Walewale.ann.rdys <- c(J,F,M,A,Ma,Ju,Jl,Au,S,O,N,D)
date.mn
monthly.rdys <- data.frame(date.mn,Walewale.ann.rdys) 
write.csv(monthly.rdys, 
          file = "Monthly Rainy Days of Walewale (1976-2017).csv")

ggplot(monthly.rdys, aes(x=date.mn, y=Walewale.ann.rdys)) + 
  geom_line(col="darkblue", lwd=2) +
  labs(title="Number of Days when Rainfall >= 0.85mm (1976-2017)\n Walewale",
       x="Month", y="Rainfall(mm)") +
...
dev.copy(png, filename="Monthly Rainy Days of Walewale grt 0.85mm (1976-2017)", 
         height = 500, width = 1000)
dev.off()

sum5 <- summary(monthly.rdys)
write.csv(sum5, 
          file="Number of Days when Rainfall grt 0.85mm of Walewale.csv")

sum55 <- data.frame(monthly.rdys[monthly.rdys[,2] == max(monthly.rdys[,2]), ],
                    monthly.rdys[monthly.rdys[,2] == min(monthly.rdys[,2]), ])

sum55[c(2,3),c(1,2)] <- sum55[c(1,2), c(3,4)] 
sum55[-c(3,4)] -> sum55
sum55[,1] <- data.frame(month.name[c(08,01,12)])
dimnames(sum55) <- list(
  c("Max:","Min:", "//"), 
  c("Month","Rainfall(mm)")
)  
...
write.csv(sum55,
          file = "summary stats for monthly rainy days (Walewale).csv")
...
width20 <- length(1976:2017) #....#length of 1960-2017)
#subsetting individual months out of the Prcp.mn.rdys data and identifying number of rainy days
...
#creating a vector with the aboves
Walewale.ann.rdys20 <- c(J.1,F.1,M.1,A.1,Ma.1,Ju.1,
...
write.csv(monthly.rdys20, 
          file = "Monthly Rainy Days of Walewale grt 20 (1976-2017).csv")

ggplot(monthly.rdys20, aes(x=date.mn, y=Walewale.ann.rdys20)) + 
  geom_line(col="darkblue", lwd=2) +
  labs(title="Number of Days when Rainfall >= 20mm (1976-2017)\n Walewale", 
       x="Month", y="Rainfall(mm)") + 
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
...
dev.copy(png, filename="Monthly Rainy Days of Walewale grt 20mm (1976-2017)", 
         height = 500, width = 1000)
dev.off()

sum6 <- summary(monthly.rdys20)
write.csv(sum6,
          file = "summary stats for momthly rainydys grt 20mm of WALEWALE.csv")
...
write.csv(sum66,
          file = "summary stats for monthly rainy days grt 20mm (Walewale).csv")
...
write.csv(Prcp.df.ann.anom, 
          file="Data for Index of Annual RAINFALL of Walewale(1976-2017).csv")
...
dev.copy(png, filename="Index of Annual Rainfall of Walewale (1976-2017).png", 
         height=750, width=1000)
dev.off()

sum7 <- summary(Prcp.df.ann.anom)
write.csv(sum7,
          file="summary stats for Rainfall Total Anomaly of WALEWALE (1976-2017).csv")
...
write.csv(sum77,
          file = "summary stats for annual Prcp Anomaly (Walewale).csv")
...
mean_daily_Walewale <- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11, 
...
Seasonal_cycle_Walewale <-data.frame(
  c(1:366),
  mean_daily_Walewale
)

write.csv(Seasonal_cycle_Walewale, 
          file = "data_for_seasonal_cycle_of_Rainfall of Walewale(1976-2017).csv")
...
ggplot(Seasonal_cycle_Walewale, aes(x=c(1:366), y=mean_daily_Walewale)) + 
  geom_line(col="darkblue", lwd=1) + 
  labs(title="Seasonal Cycle of Rainfall (1976-2017)\n Walewale", 
       x="Day", y="Rainfall(mm)") + 
...
dev.copy(png, filename="Seasonal Rainfall Cycle of Walewale (1976-2017).png", 
         height=750, width=1000)
dev.off()

sum7 <- summary(Seasonal_cycle_Walewale)
write.csv(sum6,
          file="summary stats for seasonal Cycle of WALEWALE Rainfall(1976-2017).csv")
...
#Dividing daily Prcp
head(Prcp)
Prcp.fh <- Prcp[(Prcp$dates >= "1976-01-01" &
                   Prcp$dates <= "1996-12-31"), ]

Prcp.sh <- Prcp[(Prcp$dates >= "1997-01-01" &
                   Prcp$dates <= "2017-12-31"), ]


#Dividing annual Prcp
Prcp_fh <- Prcp.df.ann[(Prcp.df.ann$date >= "1976-12-31" &
                          Prcp.df.ann$date <= "1996-12-31"), ]
write.csv(Prcp_fh,file="Annual Rainfall of Walewale(1976-1996).csv")


Prcp_sh <- Prcp.df.ann[(Prcp.df.ann$date >= "1997-12-31" &
                          Prcp.df.ann$date <= "2017-12-31"), ]
write.csv(Prcp_sh,file="Annual Rainfall of Walewale(1997-2017).csv")
...
#Mean Monthly Rainfall
head(Prcp.mn.df)
Prcp.mn.dffh <- Prcp.mn.df[(Prcp.mn.df$date >= "1976-01-31" &
                              Prcp.mn.df$date <= "1996-12-31"), ]

Prcp.mn.dfsh <- Prcp.mn.df[(Prcp.mn.df$date >= "1997-01-31" & 
...
write.csv(mean.mn.Prcpfh, 
          file="Mean Monthly Rainfall of Walewale (1976-1995).csv")

ggplot(mean.mn.Prcpfh, aes(x=date.mn, y=mean.mn.Prcpfh.v)) + 
  geom_line(col="darkblue", lwd=2) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b") + 
  labs(title="Mean Monthly Rainfall (1976-1996)\n Walewale", 
       x="Month",y="Rainfall(mm)") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Mean Monthly Rainfall of Walewale (1976-1996).png",
         height=750, width=1000)
dev.off()

sum8 <- summary(mean.mn.Prcpfh)
write.csv(sum8,
          file = "summary stats for mean monthly Rainfall of Walewale(1976-1996).csv")
...
write.csv(mean.mn.Prcpsh, 
          file="Mean mOnthly Rainfall of Walewale (1997-2017).csv")

ggplot(mean.mn.Prcpsh, aes(x=date.mn, y=mean.mn.Prcpsh.v)) + 
  geom_line(col="darkblue", lwd=2) + 
  scale_x_date(date_breaks = "1 month", date_labels = "%b") + 
  labs(title="Mean Monthly Rainfall (1997-2017)\n Walewale", 
       x="Month",y="Rainfall(mm)") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Mean Monthly Rainfall of Walewale (1997-2017.png",
         height=750, width=1000)
dev.off()

sum9 <- summary(mean.mn.Prcpsh)
write.csv(sum8,
          file = "summary Stats for mean monthly Rainfall of Walewale(1997-2017).csv")
...
sum99[-c(3,4)] -> sum99
sum99[,1] <- data.frame(month.name[c(08,12)])
dimnames(sum99) <- list(
  c("Max:","Min:"), 
  c("Month","Rainfall(mm)")
)  
...
ggplot(data=mean.mn.Prcpfh,aes(x=mean.mn.Prcpfh[,1])) + 
  geom_line(aes(y=mean.mn.Prcpfh[,2],col="1976-1996"), lwd= 2) + 
              geom_line(aes(y=mean.mn.Prcpsh[[2]], colour="1997-2017"),lwd=2) +
  scale_colour_manual("",
                      values = c("1976-1996"="darkblue","1997-2017"="red"))+
                     scale_x_date(date_breaks = "1 month", date_labels = "%b") + 
  xlab("Month") +ylab("Rainfall(mm)") + 
  ggtitle("Mean Monthly Rainfall of Walewale \n (1976-2017)") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename = "Mean Monthly Min Tmp of Walewale (1976-96-2017).png",
         width = 1000, height = 750)
dev.off()
...
#Percentage Change between both climatologies(1976-96, 1997-2017)
...
head(Ann.rndys)
Ann.rndys_fh <- Ann.rndys[Ann.rndys[,1] >= "1976-01-01" & 
                            Ann.rndys[,1] <= "1996-01-01", ]

Ann.rndys_sh <- Ann.rndys[Ann.rndys[,1] >= "1997-01-01" & 
                            Ann.rndys[,1] <= "2017-01-01", ]

#introducing another row to make Ann.rndys[,2] == Ann.rndys[,2], so we can plot both
Ann.rndys_sh[21,2] <- NA

ggplot(data = data.frame(c(1:21), Ann.rndys_fh[,2]),aes(x=c(1:21))) +
  geom_line(aes(y=Ann.rndys_fh[,2], colour = "1976-1996"), lwd=1.5) +
  geom_line(aes(y=Ann.rndys_sh[,2], colour = "1997-2017"), lwd=1.5) +
  scale_color_manual("",values = c("1976-1996"="darkblue",
                                   "1997-2017"="red")) +
...
Ann.rndys20_fh <- Ann.rdys20.Walewale[Ann.rdys20.Walewale[,1] >= "1976-01-01" &
                                        Ann.rdys20.Walewale[,1] <= "1996-01-01", ]

Ann.rndys20_sh <- Ann.rdys20.Walewale[Ann.rdys20.Walewale[,1] >= "1997-01-01" &
                                        Ann.rdys20.Walewale[,1] <= "2017-01-01", ]

#introducing another row to make Ann.rndys20.Zua[,2] == Ann.rndys20.Zua[,2], 
#so we can plot both
Ann.rndys20_sh[21,2] <- NA   

ggplot(data = data.frame(c(1:21), Ann.rndys20_fh[,2]),aes(x=c(1:21))) +
...
dev.copy(png,filename="Comparing Days with Rainfall grt 20mm Walewale.png",
         width = 1000, height = 750)
dev.off
...
head(Prcp.mn.rdys) 
Prcp.mn.rdysfh <- Prcp.mn.rdys[Prcp.mn.rdys[,1] >= "1976-01-01" &
                                 Prcp.mn.rdys[,1] <= "1996-12-31", ] %>% drop_na()

Prcp.mn.rdysh <- Prcp.mn.rdys[Prcp.mn.rdys[,1] >= "1997-01-01" &
...
Prcp.mn.rdysfh20 <- Prcp.ann.rdys20[Prcp.ann.rdys20[,1] >= "1976-12-31" &
                                      Prcp.ann.rdys20[,1] <= "1996-12-31", ] 

Prcp.mn.rdysh20 <- Prcp.ann.rdys20[Prcp.ann.rdys20[,1] >= "1997-12-31" &
...
mean_daily_Walewalefh<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11, Jan_12,
...
Seasonal_cycle_Walewalefh <-data.frame(
  c(1:366),
  mean_daily_Walewalefh
)

write.csv(Seasonal_cycle_Walewalefh, 
          file = "data_for_seasonal_cycle_of_Rainfall of Walewale (1976-1994).csv")

sss <- ksmooth(
  c(1:366), 
  mean_daily_Walewalefh, bandwidth = 15
)

sss.1 <- data.frame(sss) # converting to a dataframe

ggplot(Seasonal_cycle_Walewalefh, aes(x=c(1:366), y=mean_daily_Walewalefh)) + 
  geom_line(col="darkblue", lwd=1) + 
  labs(title="Seasonal Cycle of Rainfall (1976-1996)\n Walewale", 
       x="Day", y="Rainfall(mm)") + 
...
dev.copy(png, filename="Seasonal Cycle of Walewale (1976-1996).png", 
         height=750, width=1000)
dev.off()
...
mean_daily_Walewalesh<- c(Jan_1,Jan_2, Jan_3, Jan_4, Jan_5,Jan_6,Jan_7,Jan_8,Jan_9,Jan_10,Jan_11, Jan_12,Jan_13,Jan_14,Jan_15,Jan_16,
...
Seasonal_cycle_Walewalesh <-data.frame(
  c(1:366),
  mean_daily_Walewalesh
)

write.csv(Seasonal_cycle_Walewalesh, 
          file = "data_for_seasonal_cycle_of_Rainfall of Walewale (1997-2017).csv")

bb <- ksmooth(
  c(1:366), 
  mean_daily_Walewalesh, bandwidth = 15
)

bb.1 <- data.frame(bb) # converting to a dataframe

ggplot(Seasonal_cycle_Walewalesh, aes(x=c(1:366), y=mean_daily_Walewalesh)) + 
  geom_line(col="darkblue", lwd=1) + 
  labs(title="Seasonal Cycle of Rainfall (1997-2017)\n Walewale", 
       x="Day", y="Rainfall(mm)") + 
  scale_colour_manual("",
                      values = c("1976-1996"="darkblue","1997-2017"="red"))
...
dev.copy(png, filename="Seasonal Rainfall Cycle of Walewale (1997-2017).png",
         height=750, width=1000)
dev.off()
...
#SUOERIMPOSING
ggplot(Seasonal_cycle_Walewalefh, aes(x=Seasonal_cycle_Walewalefh[,1])) + 
  geom_line(aes(y=Seasonal_cycle_Walewalefh[,2]), lwd = .25) +
  geom_line(aes(y=sss.1[,2],colour="1976-1996"), lwd=1.5) + 
  geom_line(aes(y=Seasonal_cycle_Walewalesh[,2]),lwd=.25) +
  geom_line(aes(y=bb.1[,2],colour="1997-2017"), lwd=.25) + 
  scale_colour_manual("",
                      values = c("1976-1996"="darkblue","1997-2017"="red"))+
                        xlab("Days") + ylab("Rainfall(mm)") + 
  ggtitle("Seaonal Cycle of Rainfall (1976-2017)\n Walewale") +
  theme(plot.title = element_text(size = 18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13))

dev.copy(png, filename="Seasonal Cycle of Rainfall of Walewale(1976-96-2017).png", 
         width = 1000, height = 750)
dev.off()
...
ggplot(data=Seas,
  ...
  ggtitle(" Seasonal Rainfall Totsl (1960-2017)\n Yendi") + xlab("Month") +
  ylab("Rainfall(mm)") + 
  theme(plot.title = element_text(size=18, hjust = 0.5, face = "bold"),
        axis.text = element_text(size = 13),
        axis.title = element_text(size = 13)) +
...
#  (scale_x_date(date_breaks="4 years",date_labels="%Y"))  +
#  geom_hline(yintercept=0) + 
#  labs(title="Index of Annual Rainfall of Walewale \n (1976-1996)", 
#       x="Years", y="Index")

#dev.copy(png, filename="Index of Annual Rainfall of walewale (1976-1996).png",
#         height=750, width=1000)
#dev.off()
...
#dev.copy(png, filename="Index of Annual Rainfall of Walewale (1997-2017).png",
#         height=750, width=1000)
#dev.off()
